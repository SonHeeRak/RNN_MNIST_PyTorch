...
def define_argparser():
    ...
    p.add_argument('--rnn_hidden_size', type=int, default=64)
    p.add_argument('--rnn_n_layers', type=int, default=4)
    p.add_argument('--rnn_dropout_p', type=float, default=.2)
...
def get_model(config):
    if config.model == 'fc':
        ...
        model = SequenceClassifier(
            ...
            dropout_p=config.rnn_dropout_p,
        )
    else:
...
def main(config):
    ...
    device = torch.device('cpu') if config.gpu_id < 0 else torch.device('cuda')
